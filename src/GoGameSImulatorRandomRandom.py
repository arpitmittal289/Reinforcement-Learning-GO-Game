...
class MyGoPlayer():
    ...
    def evaluation_func(self, go):
        ...
        print("EVALUATION: " + str(my_piece_count - ta_piece_count))
...
def makeMove(piece_type, previous_board, board):
    N = 5
    go = GO(N)
...
def moveplayer(startGO,player,label,listOfMoves):
    oppPiece = PLAYER_W
    if player == PLAYER_W:
        oppPiece = PLAYER_B
    movePlayed = makeMove(player, startGO.previous_board, startGO.board)
    if movePlayed == 'PASS':
        return True
...
def playRandomGame(procId):   
    startGO = GO(5)
    startGO.init_board(5)
    listOfMoves = []
    while True:
        if moveplayer(startGO,PLAYER_B, 'b',listOfMoves):
            break
        if moveplayer(startGO,PLAYER_W, 'w',listOfMoves):
            break
...
if __name__ == "__main__":
    listOfsimulatedGames = []
    with open("listOfsimulatedGames", "rb") as fp:   # Unpickling
        listOfsimulatedGames = pickle.load(fp)
    for i in range(100):
        print("Smulating Batch "+str(i))
        pool = multiprocessing.Pool(processes = 8)
        result = pool.map(playRandomGame, range(10000))
        listOfsimulatedGames.extend(result)
        pickle.dump(listOfsimulatedGames, open("listOfsimulatedGames", "wb"), protocol=0)
